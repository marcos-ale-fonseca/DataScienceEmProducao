...
class Rossmann( object ):
    ...
    def __init__(self):
        self.path_home = '/Users/developer/Documents/Projects/repos/DataScienceEmProducao/pickles/'
        with open(self.path_home + 'm05_rescaling_competition_distance.pkl', 'rb') as file:
            self.competition_distance_scaler = pickle.load(file)
        with open(self.path_home + 'm05_rescaling_competition_time_month.pkl', 'rb') as file:
            self.competition_time_month = pickle.load(file)
        with open(self.path_home + 'm05_rescaling_promo_time_week.pkl', 'rb') as file:
            self.promo_time_week = pickle.load(file)
        with open(self.path_home + 'm05_rescaling_year.pkl', 'rb') as file:
            self.rescaling_year = pickle.load(file)
        with open(self.path_home + 'm05_encoding_store_type.pkl', 'rb') as file:
            self.encoding_store_type = pickle.load(file)
    ...
    def data_preparation( self, df5 ):
        ...
        df5['competition_distance'] = self.competition_distance_scaler.fit_transform( df5[['competition_distance']].values ) # encontra os parâmetros da fórmula, quartil 1 e quartil 3, e aplica nos dados.
    
        # competition time month
        df5['competition_time_month'] = self.competition_time_month.fit_transform( df5[['competition_time_month']].values ) # encontra os parâmetros da fórmula, quartil 1 e quartil 3, e aplica nos dados.
        
        # promo time week
        df5['promo_time_week'] = self.promo_time_week.fit_transform( df5[['promo_time_week']].values )
        
        # year
        df5['year'] = self.rescaling_year.fit_transform( df5[['year']].values )
